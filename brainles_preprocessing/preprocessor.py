...
class Preprocessor:
    ...
    def run(
        ...
        self.center_modality.register(
            self.registrator, self.atlas_image_path, self.atlas_dir, file_name
        )
        ...
        if brain_extraction:
            ...
            atlas_mask = self.center_modality.extract_brain_region(
                self.brain_extractor, bet_dir, atlas_mask, bet_mode
            )
            for moving_modality in self.moving_modalities:
                moving_modality.apply_mask(
                    self.brain_extractor, brain_masked_dir, atlas_mask
                )
